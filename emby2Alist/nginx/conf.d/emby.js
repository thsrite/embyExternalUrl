...
async function redirect2Pan(r) {
    ...
    let directUrl = fetchDirectPathApi(mediaItemPath, ua);
...
function fetchDirectPathApi(mediaItemPath, ua) {
    r.warn(`fetchDirectPathApi filePath: ${mediaItemPath}, ua: ${ua}`)
    try {
        r.warn("http://127.0.0.1:5115?path=" + encodeURI(mediaItemPath) + "&ua=" + ua)
        const response = ngx.fetch("http://127.0.0.1:5115?path=" + encodeURI(mediaItemPath) + "&ua=" + ua, {
            method: "GET",
